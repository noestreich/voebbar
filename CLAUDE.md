...
- aDIS `identity` tokens are **single-use** and page transitions are restricted: the renewability-probe result page silently ignores `*SGG` (it returns the loans page again — fees would parse as 0), the fees page ignores `*SZA`, and reusing a page's hidden inputs for a second POST lands on a session-error page. `fetchAccountData` therefore fetches loans + fees in one session and runs the probe in a **second, fresh session** (checkbox values are positional and stable across sessions); every session ends with a best-effort `*SE` logout. `applyFees` only trusts responses containing "Gebührenkonto" and otherwise sets `feesUnknown` instead of reporting 0.
