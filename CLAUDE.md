...
- Pickups ("Bereitstellungen", nav code `*SZS`) are fetched in a **third fresh session** only when the overview's service list announces them: aDIS honours exactly one list navigation per session (after `*SZA`, a `*SZS` silently returns the loans page again, and vice versa). The pickups list reuses the loans `<title>`; `HTMLParser.isPickupsPage` keys on the `Mein Konto - Bereitstellungen` heading, and rows are 4 cells (`chk`, `Bis`, `Ausgabeort`, `Titel`).
