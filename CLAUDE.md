...
- aDIS `identity` tokens are **single-use** and page transitions are restricted: the renewability-probe result page silently ignores `*SGG` (it returns the loans page again — fees would parse as 0), the fees page ignores `*SZA`, and reusing a page's hidden inputs for a second POST lands on a session-error page. `fetchAccountData` therefore navigates only overview → `*SZA` and runs the probe in a **second, fresh session** (checkbox values are positional and stable across sessions); every session ends with a best-effort `*SE` logout.
- Pickups ("Bereitstellungen", nav code `*SZS`) are fetched in a **third fresh session** only when the overview's service list announces them: a list navigation issued **from another list page** is silently ignored (after `*SZA`, a `*SZS` posted from the loans page returns the loans page again, and vice versa). The browser never does this — it always returns to the overview between lists — and that overview hop has not been replicated/tested, so the fresh session is the verified path. The pickups list reuses the loans `<title>`; `HTMLParser.isPickupsPage` keys on the `Mein Konto - Bereitstellungen` heading, and rows are 4 cells (`chk`, `Bis`, `Ausgabeort`, `Titel`).
